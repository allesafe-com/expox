...
import argparse
from expox.lib import output, request, wordlists, report, scan, extraargs, logo
import random
import time
import json
import sys
...
_params = {
    ...
    "threads": 50,                          # [int] threads num -th 50
    ...
    "output_folder": "expox_report",      # [str] report folder -o /folder/
...
class Start():
    ...
    def arguments():
        ...
        description = "-"*80+"\n"
        description += "* SCAN\n"
        description += "full scan:\texpox domain.com\n"
        description += "quick scan:\texpox domain.com --no-local\n"
        description += "faster scan:\texpox domain.com --no-local --no-http\n"
        description += "ignore code:\texpox domain.com --no-http-code 404 500 530\n"
        description += "silent mode:\texpox domain.com --silent\n\n"
        description += "* SUBDOMAINS\n"
        description += "show recon:\texpox domain.com --no-local --no-scan\n\n"
        description += "* REPORT\n"
        description += "show report:\texpox --report expox_report/domain.com_yyyy_mm_dd_hh_mm_ss.json\n"
        description += "plot report:\texpox --plot expox_report/domain.com_yyyy_mm_dd_hh_mm_ss.json\n"
        description += "csv report:\texpox --csv expox_report/domain.com_yyyy_mm_dd_hh_mm_ss.json\n"
        description += "-"*80
        
        epilog = "once you get expox results, don't forget to use 'nmap' and 'dirsearch'\n\n"
        epilog += random.choice(Start.msg_rnd())

        parser = argparse.ArgumentParser(prog="expox", description=description, epilog=epilog, 
